...
labeling_filename = 'printed_data_info'

## Check Json File
json_file = os.listdir(f'./kor_dataset/aihub_data/{data_type}/{labeling_filename}')
# print(json_file)
...
# train, validation, test set으로 나눔
for i in range(20): # json_file_number = 50
  json_file_name = json_file[i]
  # print(json_file[i])

  if json_file[i] == '.DS_Store':
    continue
  file = json.load(open(f'./kor_dataset/aihub_data/{data_type}/{labeling_filename}/{json_file_name}'))

  image = file['image']

  if image['file_name'] in train_files:
    train_img_ids.append(image['file_name'])
  elif image['file_name'] in validation_files:
    validation_img_ids.append(image['file_name'])
  elif image['file_name'] in test_files:
    test_img_ids.append(image['file_name'])

print(train_img_ids)
print(validation_img_ids)
print(test_img_ids)

# Train set을 가지고 그 json파일을 읽어서 학습시키기



image = Image.open(train_img_ids[0])
